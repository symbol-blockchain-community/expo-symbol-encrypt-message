import { encodeAesGcm, decodeAesGcm, concatArrays } from './cipherHelpers';
...
export default class MessageEncoder {
    ...
    recipientPublicKey = typeof recipientPublicKey === 'string' ? new Uint8Array(Buffer.from(recipientPublicKey, 'hex')) : recipientPublicKey;
		if (1 === encodedMessage[0]) {
			...
			if (result)
        return { isDecoded: true, message: utf8ArrayToString(message as Uint8Array) };
		}
    ...
    recipientPublicKey = typeof recipientPublicKey === 'string' ? Buffer.from(recipientPublicKey, 'hex') : recipientPublicKey;
		const { tag, initializationVector, cipherText } = encodeAesGcm(deriveSharedKey, this._privateKey, recipientPublicKey, new Uint8Array(Buffer.from(message, 'utf-8')));
